...
?>
              <li<?=$prob['lable']==$label?" class='active'":""?>><a href="#problem/<?= $prob['lable'] ?>"><?= $prob['lable'] ?></a></li>
<?php
    }
?>
            </ul>
            <h2 style="text-align:center" class="pagetitle"><?=$label.". ".$show_problem->get_val("title")?></h2>
...
<?=sidebar_contest_show($cid)?>
    ...
    <div id="submitdialog" class="modal hide fade" name="<?=$show_problem->get_val("vname")?>">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
        <h4><?= "Submit ".$prob_info["lable"].": ".htmlspecialchars($show_problem->get_val("title")) ?></h4>
      </div>
      <form action="ajax/contest_problem_submit.php" method="post" id="cprobsubmit" class="ajform form-horizontal">
        <div class="modal-body">
          <table width="100%">
            ...
            <tr style="display:none">
              <th>Label: </th>
              <td><?= $prob_info["lable"] ?><input name="lable" value="<?= $prob_info["lable"] ?>" readonly="readonly" style="display:none"></td>
            </tr>
            <tr style="display:none">
              <th>Contest: </th>
              <td><?= $cid ?><input name="contest_id" value="<?= $cid ?>" readonly="readonly" style="display:none"></td>
            </tr>
            <tr>
              <th>Language: </th>
              <td style="text-align:left;">
                <select name="language" id="lang" accesskey="l">
                  ...
                  <option value="17">GUN C++ 11</option>
                </select>
              </td>
            </tr>
            <tr>

...
function pcrawler_uva($pid){
    ...
    if (stripos($content, "<h3>") !== false){
        ...
        file_put_contents("/var/www/oj/".$pdflink, get_url("http://uva.onlinejudge.org/".$pdflink));
...
function pcrawler_uvalive($pid){
    ...
    if (stripos($content, "<h3>") !== false){
        ...
        file_put_contents("/var/www/oj/".$pdflink, get_url("https://icpcarchive.ecs.baylor.edu/".$pdflink));
        $ret["description"] = "<p><a href='$pdflink' class='bottom_link'>[PDF Link]</a></p>".trim($content);
